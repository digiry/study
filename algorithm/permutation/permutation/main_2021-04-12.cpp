...
typedef struct _User { char mName[20]; int mWeight; } User;
...
void PrintMembers(const int *const pArr, const User *const pMembers, const int length) {
	for (int i = 0; i < length; i++) {
...
void PermutationDuplicated(int *const pArr, const int n, const int r) {
	if (r == 0) {
		PrintArray(TEMP, _R);
		return;
	}
	...
	for (int i = n - 1; i >= 0; i--) {
		Swap(pArr, n - 1, i);
		TEMP[r - 1] = pArr[n - 1];
		PermutationDuplicated(pArr, n, r - 1);
		Swap(pArr, n - 1, i);
	}
}

void PermutationDuplicated(int* const pArr, const User* const pMembers, const int n, const int r) {
	if (r == 0) {
		PrintMembers(TEMP, pMembers, _R);
		return;
	}
	...
	for (int i = n - 1; i >= 0; i--) {
		Swap(pArr, n - 1, i);
		TEMP[r - 1] = pArr[n - 1];
		PermutationDuplicated(pArr, pMembers, n, r - 1);
		Swap(pArr, n - 1, i);
	}
}
...
int main() {
	...
	cout << "---" << endl;
	PermutationDuplicated(ARR, _N, _R);
	cout << "---" << endl;
	Permutation(ARR4MEMBERS, MEMBERS, _N, _R);
	cout << "---" << endl;
	PermutationDuplicated(ARR4MEMBERS, MEMBERS, _N, _R);
