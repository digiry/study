...
void init(int N, char init_string[]) {
	...
	for (int i = 0; i <= SRC_STRING_LENGTH - KEY_SIZE; ++i) {
		...
		if (it != KEY_MAP.end()) {
			...
			pSet->clear();
			pSet->insert(i);
			KEY_MAP.insert(make_pair(hash, pSet));
		}
	}
}
...
int change(char string_A[], char string_B[]) {
	...
	auto str_a_it = KEY_MAP.find(str_a_hash);
	...
	for (int index : set_to_modify) {
		for (int x = index - 2; x <= index + 2; ++x) {
			...
			auto target_it = KEY_MAP.find(target_hash);
		...
		for (int x = index - 2; x <= index + 2; ++x) {
			...
			if (target_it != KEY_MAP.end()) {
				...
				target_set->clear();
				target_set->insert(x);
				KEY_MAP.insert(make_pair(target_hash, target_set));
			}			
		}
	}
